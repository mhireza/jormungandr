...
use assert_fs::{fixture::ChildPath, NamedTempFile, TempDir};
...
pub fn assert_genesis_encode_fails(block0_configuration: &Block0Configuration, expected_msg: &str) {
    ...
    startup::write_block0_config(&block0_configuration, &config_file);
    let output_file = temp_dir.child("block-0-failure.bin");
    process_assert::assert_process_failed_and_matches_message(
        jcli_commands::get_genesis_encode_command(config_file.path(), output_file.path()),
        expected_msg,
    );
}
